...
export default function ThreeHello({ themeMode = 'light' }) {
  ...
  useEffect(() => {
    ...
    // 3. Environment Map Generator for Realistic Specular Highlights
    ...
    // 4. Physical Materials for 3D Model
    const isDark = themeMode === 'dark';

    const helloMaterial = new THREE.MeshPhysicalMaterial({
    ...
    // 5. Load the exact 3D Model GLTF from haoqi.design
    ...
    loader.load(
      ...
      (gltf) => {
        ...
        // Apply theme materials to all meshes in the model
        loadedModel.traverse((child) => {
          if (child.isMesh) {
            child.material = helloMaterial;
        ...
        // Also load the accent star/sticker if available
        loader.load('/model/cnt.gltf', (cntGltf) => {
          const cntModel = cntGltf.scene;
          cntModel.traverse((child) => {
            if (child.isMesh) {
              child.material = accentMaterial;
            }
          });
          cntModel.scale.set(0.8, 0.8, 0.8);
          cntModel.position.set(4.5, 0.8, 0.8);
          modelGroup.add(cntModel);
        }, undefined, () => {});
      },
      undefined,
      (error) => {
        console.error('Error loading hello.gltf model:', error);
      }
    );
    ...
    // 7. Drag Rotation & Mouse Tracking
  ...
  return (
    <div 
      ref={containerRef} 
      className="w-full h-full min-h-[600px] flex items-center justify-center cursor-grab active:cursor-grabbing"
    />
  );
}
